<script>
	...
	// data
	// import trapResults from "../trapResults";

	// variables
	// let flowDepthGiven = true;
	// let results = [...trapResults]; // for local copy of data
	let sdigs = 5,
		wdigs = 5,
		zl = 3,
		zr = 3,
	...
	function getArea(depth) {
		return b * depth + ((zl + zr) * depth ** 2) / 2;
	}

	function getWP(depth) {
		return b + depth * ((1 + zl ** 2) ** 0.5 + (1 + zr ** 2) ** 0.5);
	}
	...
	function getYc(Q, shallow = 0, deep = 100) {
		...
		//want NF(shallow) > 1 and NF(deep) < 1
		// while (getNFfromY(shallow + increment, Q) > 1) {
		// 	shallow += increment;
		// 	deep = shallow + increment;
		// }

		// do the search
		if (getNFfromY(mid, Q) < 1) {
			return getYc(Q, shallow, mid);
		} else {
			return getYc(Q, mid, deep);
		}
	}
...
<main>
	...
	<section class="fig">
		<img
			src="./images/trapezoidalChannelSectionQ.png"
			alt="trapezoidal channel section"
		/>
		{#if specifyY}
			<img
				class="super"
				src="./images/trapezoidalChannelSectionY.png"
			...
			<section class="normal">
				...
				<Card
					...
					solution="$$
						\begin&lcub;aligned&rcub;
							...
							&= \left({b}+&lcub;\left(\frac &lcub; {zl}+{zr} &rcub; &lcub; 2 &rcub; \right)&rcub;\cdot {y} 			\right) \cdot {y} \\
				...
				<Card
					...
					\begin&lbrace;aligned&rbrace;
						WP &= b+\left(\sqrt &lbrace; 1+z_L^2 &rbrace; + \sqrt &lbrace; 1+z_R^2 &rbrace;\right)\cdot y \\
						&= {b}+\left(\sqrt &lbrace; 1+{zl}^2 &rbrace; + \sqrt &lbrace; 1+{zr}^2 &rbrace;\right)\cdot {y} \\
			...
			<section class="critical">
				...
				<Card
					...
					\begin&lbrace;aligned&rbrace;
						...
						\Rightarrow \frac&lbrace;{Q}^2&rbrace;&lbrace;{g}&rbrace; &= \frac&lbrace;\left(\left({b}+&lcub;\left( {zl}+{zr} \right)&rcub;\cdot y_c/2 \right) \cdot y_c\right)^3&rbrace;&lbrace;{b}+({zl}+{zr})\cdot y_c&rbrace; \\
					\end&lbrace;aligned&rbrace;$$
				...
				<Card
					...
					\begin&lbrace;aligned&rbrace;
						...
						&= \left({b}+({zl}+{zr})\cdot {yc}/2\right)\cdot {yc} \\
				...
				<Card
					...
					\begin&lbrace;aligned&rbrace;
						...
						&= \left({b}+&lcub;\left( {zl}+{zr} \right)&rcub;\cdot {yc}/2 \right) \cdot {yc} \\
						&= {Ac} \,\mathsf&lbrace;m^2&rbrace;
						
						\\\\

						W\!P_c &= b+\left(\sqrt &lbrace; 1+z_L^2 &rbrace; + \sqrt &lbrace; 1+z_R^2 &rbrace;\right)\cdot y_c \\
						&= {b}+\left(\sqrt &lbrace; 1+{zl}^2 &rbrace; + \sqrt &lbrace; 1+{zr}^2 &rbrace;\right)\cdot {yc} \\
